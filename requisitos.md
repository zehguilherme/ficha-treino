...
Treino 1 ----- N Treino_Exercício

Exercício 1 ----- N Treino_Exercício
```

E uma restrição importante no banco

```
UNIQUE (treino_id, exercicio_id)
```

Ela garante exatamente esta regra:

- O mesmo exercício não pode aparecer duas vezes no mesmo treino.
...
Porque é ela que garante um treino por dia para cada usuário
