...
A Ficha de Treino é uma aplicação web simples, responsiva e acessível para gerenciamento de treinos de academia. A aplicação permite organizar exercícios por dia da semana, acompanhar o progresso e consultar um catálogo de exercícios em português brasileiro. Cada usuário tem uma rotina semanal própria, sem compartilhamento de dados entre contas.
...
- Swagger: [documentação local](http://localhost:3001/api/docs);
- Health check: [verificar API local](http://localhost:3001/api/health).

### Produção

- Frontend: [fichatreino.vercel.app](https://fichatreino.vercel.app);
- API: [API de produção](https://ficha-treino-backend.vercel.app);
- Swagger: [documentação de produção](https://ficha-treino-backend.vercel.app/api/docs);
- Health check: [verificar API de produção](https://ficha-treino-backend.vercel.app/api/health).
...
Após iniciar os dois serviços, consulte os links locais na seção [Ambientes](#-ambientes).
