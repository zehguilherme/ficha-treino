...
- **Nunca** usar tipo `any` — toda variável, parâmetro e retorno de função deve ter tipo explícito. Se o tipo necessário não existir, criar `type` ou `interface` próprio. Validar com `npm run lint && npm run format:check` em cada módulo antes de concluir.
